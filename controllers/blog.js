            ...
            if (!err && post && !S(req.body.content).isEmpty() ){
                ...
                post.save(function(err){
                    if( !err ){
                        ...
                        helpers.users.addActivity('new_blog_post_comment', activity, req.socketio);

                        // Send email
                        var email = {
                            text: util.format( res.lingua.content.notify.comment.body, post.title, res.locals.siteUrl + '/blog/' + post.slug ),
